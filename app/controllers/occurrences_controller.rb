class OccurrencesController < ApplicationController
  ...
  def index
    @occurrences = Occurrence.where(solved: false)
  end
  ...
  def create
    @occurrence = Occurrence.new(occurrence_parrams)
    @occurrence.user = current_user
    respond_to do |format|
      if @occurrence.save
        format.html { redirect_to @occurrence, notice: "Sua ocorrência de número #{@occurrence.id} foi registrada." }
  ...
  def update
    respond_to do |format|
      if @occurrence.update(occurrence_parrams)
  ...
  private
    ...
    def occurrence_parrams
      params.require(:occurrence).permit(:description, :end, :solved, :user_id, :title)
    end
end
